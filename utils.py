...
def get_boxes_from_mask(mask, box_num=1, std = 0.1, max_pixel = 5):
    """
    Args:
        mask: Mask, can be a torch.Tensor or a numpy array of binary masks.
...
def select_random_points(pr, gt, point_num = 9):
    """
    Selects random points from the predicted and ground truth masks1 and assigns labels to them.
    Args:
        pred (torch.Tensor): Predicted masks tensor.
        gt (torch.Tensor): Ground truth masks tensor.
...
def init_point_sampling(mask, get_point=1):
    """
    Initialization samples points from the masks and assigns labels to them.
    Args:
        mask (torch.Tensor): Input masks tensor.
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, mask):
        ...
        masks: [B, 1, H, W]
        """
        assert pred.shape == mask.shape, "pred and masks should have the same shape."
