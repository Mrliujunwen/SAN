...
class SharedWindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv, shared_qkv, qk_scale=None, attn_drop=0., proj_drop=0.,
                 shared_ratio=0.5):
...
class Shared_SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, qkv, shared_qkv, window_size=7, shift_size=0,
        ...
        self.attn = SharedWindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
            qkv=qkv, shared_qkv=shared_qkv, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        for i in range(depth):
            ...
            swinblock = Shared_SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                                    ...
                                                    shared_qkv=shared_qkv, window_size=window_size,
                                                    shift_size=0 if (i % 2 == 0) else window_size // 2,
                                                    mlp_ratio=mlp_ratio,
                                                    qk_scale=qk_scale,
                                                    drop=drop, attn_drop=attn_drop,
...
class TransNuSeg(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3, num_classes=2,
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer < self.num_layers - 1:
                ...
                ##### bottleneck
                layer = shiftedBlock(
                    dim=int(embed_dim * 2 ** i_layer), num_heads=num_heads[i_layer], mlp_ratio=1, qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up2 = FinalPatchExpand_X4(input_resolution=(img_size // patch_size, img_size // patch_size),
                                           dim_scale=4, dim=embed_dim)
            self.output2 = nn.Conv2d(in_channels=embed_dim, out_channels=self.num_classes, kernel_size=1, bias=False)

            self.up3 = FinalPatchExpand_X4(input_resolution=(img_size // patch_size, img_size // patch_size),
                                           dim_scale=4, dim=embed_dim)
    ...
    def forward(self, x):
        ...
        seg_mask, edge_mask, cluster_edge = self.forward_up_features(seg_mask, edge_mask, seg_mask_downsample,
                                                                     edge_mask_downsample)
